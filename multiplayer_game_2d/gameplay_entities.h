...
{
  enum class type_and_sprite_sheet_row_index : int
  {
    NONE  = 0,
    MARIO = 1,
    BOMB  = 2
  ...
  type_and_sprite_sheet_row_index types[p_max_size] = {type_and_sprite_sheet_row_index::NONE}; // type of gameplay entity and used to specify row in sprite_sheet
  ...
  void update_positions_and_tex_coords(const float elapsed_frame_time_seconds)
  {
    ...
    for(int i=0,vertex=0; i < max_size; ++i,vertex += 4)
    {
      sprite_sheet_y_position = type_and_sprite_sheet_row_index[i] * sprite_sheet_side_length * is_garbage_flags[i];
      sprite_sheet_x_position = animation_indexes[i] * sprite_sheet_side_length * is_garbage_flags[i];
